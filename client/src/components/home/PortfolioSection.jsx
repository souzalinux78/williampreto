    ...
    <section id="portfolio" className="py-24 md:py-36 bg-white relative">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16">
           ...
           <Link to="/portfolio" className="hidden md:inline-block border-b border-primary-900 text-primary-900 uppercase tracking-widest text-xs font-medium pb-2 hover:text-primary-600 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {images.slice(0, 4).map((img, idx) => {
             const categoryName = img.title || img.category?.name || 'Todas';
             return (
               <Link key={img.id || idx} to={`/portfolio?cat=${encodeURIComponent(categoryName)}`}>
                 <motion.div 
        ...
        <div className="mt-12 text-center md:hidden">
           <Link to="/portfolio" className="inline-block border-b border-primary-900 text-primary-900 uppercase tracking-widest text-xs font-medium pb-2 hover:text-primary-600 transition-colors">
             Ver Portfólio Completo
           </Link>
        </div>
      </div>
