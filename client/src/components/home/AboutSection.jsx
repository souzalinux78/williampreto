...
const AboutSection = ({ heroImage, sectionData }) => {
  // Use image from CMS OR fallback to the original maternity photof
  const finalImage = sectionData?.aboutImage ? (sectionData.aboutImage.startsWith('/uploads') ? `${import.meta.env.VITE_API_URL.replace('/api', '')}${sectionData.aboutImage}` : sectionData.aboutImage) : "/maternity.png";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          >
            <div className="aspect-[4/5] bg-primary-200 shadow-xl overflow-hidden rounded-sm relative z-10 w-4/5 ml-auto">
               <img src={finalImage} alt="A Essência" className="w-full h-full object-cover" />
            </div>
